...
public class OrderController {
    ...
    public ResponseEntity<ResponseApi<List<OrderItemDto>>> getOrderByNumber(@PathVariable String orderNumber) {
        List<OrderItemDto> orderItems = orderService.getOrderByNumber(orderNumber);
        return ResponseEntity.ok(ResponseApi.success("주문 조회 성공", orderItems));
    }

    //회원 주문 조회
    @GetMapping("/member/{memberId}")
    public ResponseEntity<ResponseApi<List<OrderResponseDto>>> getOrdersByMember(@PathVariable Long memberId) {
        List<OrderResponseDto> orderItems = orderService.getOrderByMember(memberId);
        return ResponseEntity.ok(ResponseApi.success("회원별 주문 조회 성공", orderItems));
    }
